...
void PX4CtrlFSM::process()
{
	...
	switch (state)
	{
	// 在启动run_ctrl.launch之前，一定要先将遥控器拨到5，6通道，不然就会走mannual_ctrl中第一个if的流程 Try to jump to AUTO_HOVER
	// 第一个if用于手控定点飞行
	case MANUAL_CTRL:
	{
		// ROS_INFO("In case MANNUAL_CTRL, enter_hover_mode: %d, is_hover_mode: %d, enter_command_mode: %d, is_command_mode: %d, toggle_reboot: %d",
        //      rc_data.enter_hover_mode, rc_data.is_hover_mode, 
        //     rc_data.enter_command_mode, rc_data.is_command_mode, rc_data.toggle_reboot);
	...
	case AUTO_HOVER:
	{
		// ROS_INFO("In case AUTO_HOVER, enter_hover_mode: %d, is_hover_mode: %d, enter_command_mode: %d, is_command_mode: %d, toggle_reboot: %d",
        //      rc_data.enter_hover_mode, rc_data.is_hover_mode, 
        //     rc_data.enter_command_mode, rc_data.is_command_mode, rc_data.toggle_reboot);
	...
	case CMD_CTRL:
	{
		// ROS_INFO("In case CMD_CTRL, enter_hover_mode: %d, is_hover_mode: %d, enter_command_mode: %d, is_command_mode: %d, toggle_reboot: %d",
        //      rc_data.enter_hover_mode, rc_data.is_hover_mode, 
        //     rc_data.enter_command_mode, rc_data.is_command_mode, rc_data.toggle_reboot);

		// 为了避免在仿真中切换除指令控制模式，将下面状态转换的代码注释

		// 如果当前不处于定点模式或者当前没有收到里程计数据，则进入手动模式
		// if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		// {
		// 	state = MANUAL_CTRL;
		// 	toggle_offboard_mode(false);
		...
		// }
		// 如果当前不处于指令控制模式或者当前没有收到里程计数据，则进入定点模式
		// else if (!rc_data.is_command_mode || !cmd_is_received(now_time))
		// {
		// 	state = AUTO_HOVER;
		// 	set_hov_with_odom();
	...
	case AUTO_TAKEOFF:
	{
		...
		{
			state = AUTO_HOVER;
			// 达到起飞高度后，设置定点（为达到起飞高度时里程计输出的点），在没有进入指令控制模式之前，稳定在这一点
			set_hov_with_odom();
...
// 通过遥控器的前四个通道设置定点飞行的位置和航向角
// 前四个通道不在初始位置（通道值为0）的情况下，可能有意想不到的结果
...
{
	return (now_time - cmd_data.rcv_stamp).toSec() < param.msg_timeout.cmd; //当前时间和接收到cmd_data的时间小于0.5s为true
}
...
bool PX4CtrlFSM::toggle_offboard_mode(bool on_off)
{
	...
	if (on_off)
	{
		...
		offb_set_mode.request.custom_mode = "OFFBOARD"; // OFFBOARDz是自定义的状态，call service之后返回值如下所示：
		bool offboard_success = set_FCU_mode_srv.call(offb_set_mode);
		if (!(offboard_success && offb_set_mode.response.mode_sent))
		{	
			ROS_INFO("srv return: %d", offboard_success); // 0 失败
			ROS_INFO("offb_set_mode.response.mode_sent: %d", offb_set_mode.response.mode_sent); // 0 失败
