...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    {   // 如果last_reboot_cmd小于阈值，且reboot_cmd大于阈值，则说明当前命令在让飞机重启飞控
    ...
    last_gear = gear;
    last_reboot_cmd = reboot_cmd;
    ROS_INFO("new rc input, enter_hover_mode: %d, is_hover_mode: %d, enter_command_mode: %d, is_command_mode: %d, toggle_reboot: %d",
             enter_hover_mode, is_hover_mode, 
            enter_command_mode, is_command_mode, toggle_reboot);
}

void RC_Data_t::check_validity()
{
    // -1.1 <= mode <= 1.1, -1.1 <= gear <= 1.1, -1.1 <= reboot_cmd <= 1.1
    // 通道值取2000恰好为1 
